...
#----------------------------------------Button初始化部分----------------------------------------
    ...
    def button_with_text(self, index):
        ...
        # 将文本进行换行处理
        text_lines = textwrap.wrap(self.buttons_with_text[index]['label'], width=5, 
    ...
    def execute_button_action(self, index):
        ...
        if index == 0:
            if self.start_search:
                ...
                self.buttons_with_text[index]['label'] = 'Restart\nSearch'
                self.buttons_with_text[index]['callback']  = lambda: setattr(self, 'restart_search', True)
            
            if self.restart_search or self.state_changed:
                if self.restart_search:
                    self.search_signal = True
                self.buttons_with_text[index]['label'] = 'Start\nSearch'
                self.buttons_with_text[index]['callback']  = lambda: setattr(self, 'start_search', True)
            ...
            if self.resume_search:
                ...
                self.buttons_with_text[index+1]['label'] = 'Pause\nSearch'
                self.buttons_with_text[index+1]['callback']  = lambda: setattr(self, 'pause_search', True)

        if index == 2:
            if self.start_search or self.restart_search or self.resume_search:
                if self.pause_search:
                    self.search_signal = False
                    self.buttons_with_text[index]['label'] = 'Cancel\nSearch'
                    self.buttons_with_text[index]['callback']  = lambda: setattr(self, 'cancel_search', True)

                    # 点击暂停后, start_search和restart_search
                    # 此时button 1文字也对应修改
                    self.buttons_with_text[index-1]['label'] = 'Resume\nSearch'
                    self.buttons_with_text[index-1]['callback']  = lambda: setattr(self, 'resume_search', True)
        ...
        if index == 2:
            ...
            if self.init_walls:
                ...
                self.buttons_with_text[index]['label'] = 'Clear\nWalls'
                self.buttons_with_text[index]['callback']  = lambda: setattr(self, 'clear_walls', True)
            
            if self.clear_walls:
                # self.clear_walls = False 这一步在执行完障碍物初始化后执行，在gridmap中
                self.buttons_with_text[index]['label'] = 'Init\nWalls'
                self.buttons_with_text[index]['callback']  = lambda: setattr(self, 'init_walls', True)
        ...
        if all(not button['rect'].collidepoint(mouse_pos) for button in self.buttons_with_text):
            ...
            for button in self.buttons_with_text:
                if button['rect'].collidepoint(mouse_pos):
                    button_index = self.buttons_with_text.index(button)
                    button['callback']()
                    
                    self.execute_button_action(button_index)
                    button = self.button_with_text(button_index)
                    self.button_surfaces[button_index] = button
                    break
